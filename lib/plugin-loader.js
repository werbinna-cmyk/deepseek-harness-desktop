...
/** The plugins root (dev vs packaged). Plugins ship inside app.asar (files), so
 *  the packaged root is app.getAppPath()/plugins — Electron's asar virtual FS
 *  serves readdir/require/loadFile transparently. */
function pluginsDir() {
  const { app } = require('electron');
  if (app.isPackaged) return path.join(app.getAppPath(), 'plugins');
